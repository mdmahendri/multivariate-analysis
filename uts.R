...
mean_custom <- function(column) {
    if(is.data.frame(data3)) {
        data3 <- data3[,unlist(lapply(data3, is.numeric))]
    }
    mean_vec <- sapply(
        column,
...
to_compare <- c('LIMIT_BAL', 'BILL_AMT1', 'BILL_AMT2')
data3 <- data2[,to_compare]
mean_vec <- mean_custom(data3)
mean_vec <- sapply(
    data3,
    function(x) Reduce('+', x) / length(x)
)
...
#no2
list1 <- c(8.8,9.6,9.3,8.5,8.7,9.5,9.2,9.6,9.6,10.1,9.7,9.2)
list2 <- c(73.8,75.4,74.2,73.3,75.1,75.4,76.3,77.2,74.3,76.5,75,76)
list3 <- c(rep('group1',4),rep('group2',4),rep('group3',4))
my_data <- data.frame(list3,list1,list2)
oneway_manova(my_data, my_data$list3, c('list1', 'list2'))

data2 <- read.csv('DataUTS2018.csv', header = TRUE, skip = 1)

#no3
onesample_t2(data2, c('LIMIT_BAL', 'BILL_AMT1', 'BILL_AMT2'), c(170000,44000,42000))

#no4
twosample_t2(data2, data2$MARRIAGE, c('1','2'), c('BILL_AMT1', 'BILL_AMT2'))